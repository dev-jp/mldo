 ...
 * @params: lambdaARN
 * @params: value: number of memory to deploy, e.g. 128, 256
 * @params: alias: string "RAM"+memory, e.g. "RAM128", "RAM256"...
 */
...
module.exports.setLambdaPower = (lambdaARN, value) => {
    ...
    const lambda = utils.lambdaClientFromARN(lambdaARN);
    return lambda.updateFunctionConfiguration(params).promise();//PP, https://docs.aws.amazon.com/lambda/latest/dg/API_UpdateFunctionConfiguration.html
};
...
//
module.exports.mldoInvokeLambda = (lambdaARN, alias, payload) => {
    console.log(`Invoking function ${lambdaARN}:${alias || '$LATEST'} with payload ${JSON.stringify(payload)}`);
    const params = {
        FunctionName: lambdaARN,
        Qualifier: alias,
